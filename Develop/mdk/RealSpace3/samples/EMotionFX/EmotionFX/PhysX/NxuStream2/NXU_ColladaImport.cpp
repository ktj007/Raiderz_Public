...
static float getFloat(const	char *s)
{
		...
		if (s)
		{
				if (_stricmp(s, "FLT_MIN")	== 0 ||	_stricmp(s, "-INF") ==	0)
				{
						ret	=	FLT_MIN;
				}
				else if	(_stricmp(s,	"FLT_MAX") ==	0	|| _stricmp(s,	"INF") ==	0)
				{
						ret	=	FLT_MAX;
				}
				else
		...
		void get(NxJointDriveDesc	&d)const
		{
				switch (driveType)
				{
						...
						case JDT_POSITION_VELOCITY:
							d.driveType	=	(NxD6JointDriveType) (NX_D6JOINT_DRIVE_POSITION	|	NX_D6JOINT_DRIVE_VELOCITY);
							break;
				}
...
class	C_RigidConstraint: C_Base
{
	public:
		...
		void setJointDriveDesc(TiXmlElement	*element)
		{
				...
				if (id)
				{
						if (_stricmp(id,	"xDrive")	== 0)
						{
								mDrive = &xDrive;
						}
						else if	(_stricmp(id, "yDrive") ==	0)
						{
								mDrive = &yDrive;
						}
						else if	(_stricmp(id, "zDrive") ==	0)
						{
								mDrive = &zDrive;
						}
						else if	(_stricmp(id, "swingDrive") ==	0)
						{
								mDrive = &swingDrive;
						}
						else if	(_stricmp(id, "twistDrive") ==	0)
						{
								mDrive = &twistDrive;
						}
						else if	(_stricmp(id, "slerpDrive") ==	0)
						{
								mDrive = &slerpDrive;
						}
				}
		...
		virtual	void setText(CELEMENT	operation, const char	*data)
		{
				switch (operation)
				{
						case CE_DRIVE_TYPE:
							if (mDrive)
							{
									if (_stricmp(data,	"NX_D6JOINT_DRIVE_POSITION") ==	0)
									{
											mDrive->driveType	=	JDT_POSITION;
									}
									else if	(_stricmp(data, "NX_D6JOINT_DRIVE_VELOCITY")	== 0)
									{
											mDrive->driveType	=	JDT_VELOCITY;
									}
									else if	(_stricmp(data, "NX_D6JOINT_DRIVE_POSITION+NX_D6JONT_DRIVE_VELOCITY") ==	0)
									{
...
void C_RigidConstraint::getJoint(NxSceneDesc	*scene,	C_Query	*q,	C_PhysicsModel *pmodel)
{
	...
	if (mProjectionMode)
	{
		if (_stricmp(mProjectionMode, "NX_JPM_POINT_MINDIST") ==	0)
		{
