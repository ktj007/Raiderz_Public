...
public:
  ...
  NodeType getElementType(const char *str)
  {
  	NodeType ret = NT_NONE;
		...
		if ( stricmp(str,"SceneGraph") == 0 )  ret = NT_SCENE_GRAPH;
		else if ( stricmp(str,"Mesh") == 0 )  ret = NT_MESH;
		else if ( stricmp(str,"Animation") == 0 )  ret = NT_ANIMATION;
		else if ( stricmp(str,"Skeleton") == 0 )  ret = NT_SKELETON;
		else if ( stricmp(str,"Bone") == 0 )  ret = NT_BONE;
		else if ( stricmp(str,"MeshSection") == 0 )  ret = NT_MESH_SECTION;
		else if ( stricmp(str,"VertexBuffer") == 0 )  ret = NT_VERTEX_BUFFER;
		else if ( stricmp(str,"TetraMesh") == 0 )  ret = NT_NODE_TETRA_MESH;
		else if ( stricmp(str,"IndexBuffer") == 0 )  ret = NT_INDEX_BUFFER;
		else if ( stricmp(str,"NodeTriangle") == 0 )  ret = NT_NODE_TRIANGLE;
		else if ( stricmp(str,"NodeInstance") == 0 )  ret = NT_NODE_INSTANCE;
		else if ( stricmp(str,"AnimTrack") == 0 )  ret = NT_ANIM_TRACK;
	...
	AttributeType getAttributeType(const char *str)
	{
		...
		if ( stricmp(str,"name") == 0 ) ret = AT_NAME;
		else if ( stricmp(str,"count") == 0 ) ret = AT_COUNT;
		else if ( stricmp(str,"parent") == 0 ) ret = AT_PARENT;
		else if ( stricmp(str,"material") == 0 ) ret = AT_MATERIAL;
		else if ( stricmp(str,"ctype") == 0 ) ret = AT_CTYPE;
		else if ( stricmp(str,"semantic") == 0 ) ret = AT_SEMANTIC;
		else if ( stricmp(str,"position") == 0 ) ret = AT_POSITION;
		else if ( stricmp(str,"orientation") == 0 ) ret = AT_ORIENTATION;
		else if ( stricmp(str,"duration") == 0 ) ret = AT_DURATION;
		else if ( stricmp(str,"dtime") == 0 ) ret = AT_DTIME;
		else if ( stricmp(str,"trackcount") == 0 ) ret = AT_TRACK_COUNT;
		else if ( stricmp(str,"framecount") == 0 ) ret = AT_FRAME_COUNT;
