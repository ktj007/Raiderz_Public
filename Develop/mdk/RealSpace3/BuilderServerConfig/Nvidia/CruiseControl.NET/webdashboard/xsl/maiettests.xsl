<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/TR/xhtml1/strict">
	...
	<xsl:template match="/">
		...
		<xsl:variable name="maiettest.report.list" select="$maiettest.result.list/maietreport"/>
		<xsl:variable name="maiettest.case.list" select="$maiettest.result.list/maiettest"/>
		...
		<table class="section-table" cellpadding="2" cellspacing="0" border="0" width="98%">
			...
			<xsl:call-template name="maiettest__reportdetail">
				<xsl:with-param name="detailnodes" select="//maietreport"/>
			</xsl:call-template>
		</table>
		...
		<table class="section-table" cellpadding="2" cellspacing="0" border="0" width="98%">
			...
			<xsl:call-template name="maiettest__testdetail">
				<xsl:with-param name="detailnodes" select="//maiettest"/>
			</xsl:call-template>
		</table>
	</xsl:template>
	...
	<xsl:template name="maiettest__testdetail">
		...
		<xsl:for-each select="$detailnodes">
			...
			<tr>
				<td class="section-data" width="100">Test:</td><td class="section-data"><xsl:value-of select="@name"/></td></tr>
			<tr>
				<td class="section-data">Time:</td><td class="section-data"><xsl:value-of select="@time"/></td></tr>
