...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/TR/html4/strict.dtd">	
	...
	<xsl:template match="/">
		<xsl:variable name="tests.root" select="//testsuites" />
		<div id="report">
			<script>
				...
				<table>
					<tbody>
						...
						<tr>
							...
							<td><xsl:value-of select="count($tests.root//testcase)"/></td>
						</tr>
						<tr>
							<td>Passes:</td>
							<td><xsl:value-of select="count($tests.root//testcase)-count($tests.root//testcase[@status = 'run' and failure])"/></td>
						</tr>
						<tr>
							<td>Fails:</td>
							<td><xsl:value-of select="count($tests.root//testcase[@status = 'run' and failure])"/></td>
						</tr>
						<tr>
							<td>Ignored:</td>
							<td><xsl:value-of select="count($tests.root//testcase[@status = 'notrun'])"/></td>
						</tr>
					</tbody>
				</table>
			</div>
	...
	<xsl:template match="testsuites">
		...
		<xsl:variable name="failure.count" select="count(.//testcase[@status='run' and failure])" />
		<xsl:variable name="ignored.count" select="count(.//testcase[@status='notrun'])" />
		...
		<div>
			...
			<blockquote>
				<table>
					...
					<xsl:apply-templates select=".//testsuite[@failures!='0'][testcase]">
						<xsl:sort select="@name" order="ascending" data-type="text"/>
					</xsl:apply-templates>
					<xsl:apply-templates select=".//testsuite[@failures='0' and @disabled!='0'][testcase]">
						<xsl:sort select="@name" order="ascending" data-type="text"/>
					</xsl:apply-templates>
					<xsl:apply-templates select=".//testsuite[@failures='0' and @disabled='0'][testcase]" mode="success">
						<xsl:sort select="@name" order="ascending" data-type="text"/>
					</xsl:apply-templates>
				</table>
			</blockquote>
	...
	<xsl:template match="testsuite" mode="success">
		<xsl:if test="count(testcase[@status='run' and failure]) + count(testcase[@status='notrun']) = 0">
			<xsl:apply-templates select="."/>
		</xsl:if>
	</xsl:template>
	
	<xsl:template match="testsuite">
		<xsl:variable name="passedtests.list" select="testcase[@status='run' and not(failure)]"/>
		<xsl:variable name="ignoredtests.list" select="testcase[@status='notrun']"/>
		<xsl:variable name="failedtests.list" select="testcase[@status='run' and failure]"/>
		<xsl:variable name="tests.count" select="count(testcase)"/>
	...
	<xsl:template match="testcase[@status='run' and not(failure)]">
		<tr>
  ...
  <xsl:template match="testcase[@status='run' and failure]">
		<tr>
			<xsl:if test="position() mod 2 = 0">
				<xsl:attribute name="class">section-oddrow</xsl:attribute>
			</xsl:if>
	...
	<xsl:template match="testcase[@status='notrun']">
		<tr>
			<xsl:if test="position() mod 2 = 0">
				<xsl:attribute name="class">section-oddrow</xsl:attribute>
			</xsl:if>
