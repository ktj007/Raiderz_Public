...
public :
	...
	static	void RegisterProperty(	const char* szName, typename CProperty<T, PropertyType>::GetterType		Getter,
														...
														typename CProperty<T, PropertyType>::GetterType		MinGetter = 0,	
														typename CProperty<T, PropertyType>::GetterType		MaxGetter = 0,
														const char* szDescription = "",
														const char* szCategory= DEFAULT_CATEGORY_NAME,
														PROPERTY_EDITOR ePropertyEditor = PE_NONE
														)
	{
		CProperty<T, PropertyType>* pProperty = new CProperty<T, PropertyType>( szName, Getter, Setter ,MinGetter,MaxGetter, szDescription, szCategory, ePropertyEditor);

		if(ePropertyEditor==PE_COLOR)
			pProperty->SetUserType(eptCOLOR);
...
template <class T, class BaseClass, ClassID CLID> CRTTI CRTTIClass<T, BaseClass, CLID>::ms_RTTI
	( 0, CLID, typeid(T).raw_name(), BaseClass::GetClassRTTI(), (ClassFactoryFunc)T::Create, 
