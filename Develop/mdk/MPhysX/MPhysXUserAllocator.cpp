...
MPhysXUserAllocator::MPhysXUserAllocator() : mNbAllocatedBytes(0), mHighWaterMark(0), mTotalNbAllocs(0), mNbAllocs(0), mNbReallocs(0)
{
...
MPhysXUserAllocator::~MPhysXUserAllocator()
{
...
void MPhysXUserAllocator::reset()
{
	LockAlloc();
...
void* MPhysXUserAllocator::malloc(size_t size)
{
	//printf("Obsolete code called!\n");
	return NULL;
}

void* MPhysXUserAllocator::malloc(size_t size, NxMemoryType type)
{
#ifdef _DEBUG
...
void* MPhysXUserAllocator::mallocDEBUG(size_t size, const char* file, int line)
{
	//printf("Obsolete code called!\n");
	return NULL;
}

void* MPhysXUserAllocator::mallocDEBUG(size_t size, const char* file, int line, const char* className, NxMemoryType type)
{
#ifndef _DEBUG
...
void* MPhysXUserAllocator::realloc(void* memory, size_t size)
	{
...
void MPhysXUserAllocator::free(void* memory)
{
	if(memory == NULL)
	{
