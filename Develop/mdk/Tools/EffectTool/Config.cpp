...
void CMostRecentlyUsedList::Add(const wxString& str)
{
	// 이미 목록에 있으면 목록에 있는 아이템 제거
	for( vector<wxString>::iterator it = m_list.begin(); it != m_list.end(); ++it )
	{
		if( (*it) == str )
		{
			m_list.erase(it);
...
wxString CMostRecentlyUsedList::Get(size_t index)
{
...
void CConfig::AddEluFileToMRUList(const wxString& fileName)
{
...
void CConfig::Init()
{
	...
	wchar_t szModuleFilePath[_MAX_DIR] = {0,};
	...
	wchar_t szDataFullPath[MAX_PATH] = {0,};
	DWORD dwResult = GetCurrentDirectory( MAX_PATH, szDataFullPath );
	_ASSERT( dwResult > 0 );
	wxString strDataFullPath(szDataFullPath);
	...
	// 파일 시스템 초기화
	wxString fileSystemInitPath = szModuleFilePath;
	MCreateFileSystem( fileSystemInitPath.c_str(), MFILEACCESS_GENERIC, m_strFileSystemPath.c_str() );
}
