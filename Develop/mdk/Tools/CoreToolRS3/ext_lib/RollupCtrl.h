...
public:
	...
	void	RemovePage(CDialog* pwndTemplate);
	void	RemoveAllPages();

	void	ExpandPage(int idx, BOOL bExpand=TRUE, BOOL bScrollToPage=TRUE, BOOL bManaged = FALSE);	//idx is a zero-based index
	...
	//by pok
	void	SetTopGap(int _gap); ///< 스크롤바를 위한 위치정보
	void	FoldAllExcept(int _page); ///< 특정 페이지만을 제외하고 닫기
...
protected:
	...
	//by pok
	void GetCurrentRect_(LPRECT _pRect);
	int  m_nTopGap_;
