...
void CRollupCtrl::RemovePage( CDialog* pwndTemplate )
{
	int nSize = m_PageList.GetSize();
	int nIndex = -1;
	for (int i = 0; i < nSize; ++i)
	{
		if (m_PageList[i]->pwndTemplate == pwndTemplate)
		{
			nIndex = i;
			break;
		}
...
// Description	  : 
//---------------------------------------------------------------------------
void CRollupCtrl::ExpandPage(int idx, BOOL bExpand, BOOL bScrollToPage, BOOL bManaged)
{
	if (idx>=m_PageList.GetSize() || idx<0)				return;
	...
	if(bExpand)
	{
		m_PageList[idx]->pwndTemplate->SendMessage(WM_USER_SIDEPANEL_EXPANDED);
		if(!bManaged)
		{
			WPARAM wndId = (WPARAM)m_PageList[idx]->pwndTemplate;
			AfxGetMainWnd()->SendMessage(WM_USER_SIDEPANEL_EXPANDED, (WPARAM)idx, (LPARAM)this);
		}
	}
...
void CRollupCtrl::ExpandAllPages(BOOL bExpand)
{
	...
	{
		for (int i=0; i<m_PageList.GetSize(); i++)
			m_PageList[i]->pwndTemplate->SendMessage(WM_USER_SIDEPANEL_EXPANDED);

		AfxGetMainWnd()->SendMessage(WM_USER_SIDEPANEL_EXPANDED, NULL, NULL);
...
void	CRollupCtrl::_ExpandPage(RC_PAGEINFO* pi, BOOL bExpand)
{
	...
	if (bExpand){
		m_PageHeight+=tr.Height();
		//여기에서 보내주면 안되고 모두 그린후 recallayout을 해준후에 보내준다
		//pi->pwndTemplate->SendMessage(WM_USER_SIDEPANEL_EXPANDED);
	}else{
		m_PageHeight-=tr.Height();
	}
}
	...
	if (_this->m_PageHeight>r.Height())	//Can Scroll ?
	{
		...
		switch (uMsg) {
			...
			case WM_MOUSEMOVE: {
				...
				if(_this->m_bUseWndResizer == true)
					break;
			...
			case WM_USER_WND_RESIZER_ENABLE:
				{
					_this->m_bUseWndResizer = true;
				}
				break;
			case WM_USER_WND_RESIZER_DISABLE:
				{
					_this->m_bUseWndResizer = false;
				}
				break;
...
void CRollupCtrl::OnContextMenu( CWnd* /*pWnd*/, CPoint pos )
{
	...
	if (m_cmenuCtxt.CreatePopupMenu())
	{
		...
		m_cmenuCtxt.AppendMenu(MF_STRING,		RC_MID_EXPANDALL,		"모두 펼치기"	);
		m_cmenuCtxt.AppendMenu(MF_STRING,		RC_MID_COLLAPSEALL,	"모두 닫기"	);
...
void CRollupCtrl::GetCurrentRect_(LPRECT _pRect)
{
	this->GetClientRect(_pRect);
	RECT r;
	CWnd* pWnd = AfxGetMainWnd();
	if(pWnd != NULL)
	{
		pWnd->GetClientRect(&r);
		_pRect->bottom = r.bottom - m_nTopGap_;
	}
}

void CRollupCtrl::SetTopGap(int _gap){
	m_nTopGap_ = _gap;
}

void CRollupCtrl::FoldAllExcept(int _page)
{
	//fold except _page
	for (int i=0; i<m_PageList.GetSize(); i++)
	{
		if( _page != i)
			_ExpandPage(m_PageList[i], FALSE);
	}
