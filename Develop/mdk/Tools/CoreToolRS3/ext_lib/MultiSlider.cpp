...
void CMultiSlider::OnPaint() 
{
	...
	//by pok
	//AfxGetMainWnd()->SendMessage(WM_USER_MULTISLIDER_CHANGED);
	GetParent()->SendMessage(WM_USER_MULTISLIDER_CHANGED);
}
...
void CMultiSlider::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(sliderRect.PtInRect(point))
	{
		...
		if(m_pdraggedMarker == NULL && ActualMarkersNum < m_MarkersNum)	//Adding interval
		{
			...
			//by pok, 슬라이더를 정수형으로 바꾸려는 삽질
			int temp_a = (int)(RoundTo(x, 1));
			x = (float)temp_a;
			//삽질 끝
...
//by pok
void CMultiSlider::MakeMarker(float _position){
	int ActualMarkersNum = (int)m_Markers.GetCount();
	if(ActualMarkersNum < m_MarkersNum){
		m_pnewMarker = new CMarker/*(UpdateIndexes(x))*/;	// MLTSLD_PREVENT:CROSSING SUPPORT...

		//x = RoundTo(x, 1);
		m_pnewMarker->m_Pos = _position;
		SetRectangles(m_pnewMarker);
...
void CMultiSlider::ChangeHeadAndTailMarker(float _fHead, float _fTail){
	CMarker* pHeadMarker = (CMarker*)m_Markers.GetHead();
	pHeadMarker->m_Pos = _fHead;
	SetRectangles(pHeadMarker);

	CMarker* pTailMarker = (CMarker*)m_Markers.GetTail();
	pTailMarker->m_Pos = _fTail;
	SetRectangles(pTailMarker);
	Invalidate();
}
//end pok
...
void CMultiSlider::Initialize()
{
	...
	//by pok, 슬라이더를 정수형으로 바꾸려는 삽질
	int temp_a = (int)((max - min) / (GetNumTics()-1) + .5);
   	m_nAlignment = float(temp_a);
	//삽질 끝
	width = max-min + 1;
