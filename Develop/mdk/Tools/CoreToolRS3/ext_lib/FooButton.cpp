...
CMapStringToPtr FooButton::m_topBtnGroupGroups;                   // button groups
...
{
  POSITION pos = m_topBtnGroupGroups.GetStartPosition();
  while (pos != NULL) {
    CString strGroupName;
    void* pGroup = NULL;
    m_topBtnGroupGroups.GetNextAssoc (pos, strGroupName, pGroup);
    ASSERT (pGroup != NULL);
  ...
  (FooButton::Type type)
{
  ...
  switch (type) {
    ...
    case pushButtonDropDown:
      m_bDropDown = true;
      break;

    case hotCheckButton:
	case hotCustomRadioButton:
    case hotPushButton:
      m_bHot = true;
      break;

    case hotPushButtonDropDown:
      m_bHot = true;
      m_bDropDown = true;
      break;

    case pushButtonMulti:
      m_bMulti = true;
      break;

    case hotPushButtonMulti:
      m_bHot = true;
      m_bMulti = true;
      break;

	case hotCustomPushButtonMulti:
	  m_bHot = true;
	  m_bMulti = true;
	  break;
  ...
  // Reset checked mode
  if ((m_type != checkButton) &&
      (m_type != hotCheckButton) && (m_type != hotCustomRadioButton) && (m_type != hotCustomPushButtonMulti))
     m_bChecked = false;
...
{
  if ((m_type == checkButton) ||
      (m_type == hotCheckButton) ||
      (m_type == checkBox) ||
	  (m_type == hotCustomRadioButton) ||
	  (m_type == hotCustomPushButtonMulti) ||
      (m_type == radio)) {
      m_bChecked = bChecked;
      HWND hWnd = GetSafeHwnd();
      if ((hWnd != NULL) && ::IsWindow (hWnd))
  ...
  if (isChecked() && !m_strButtonGroup.IsEmpty()) {
      ...
      ASSERT (pGroup != NULL);
      int nButtons = (int)((CPtrArray *) pGroup)->GetSize();
...
{
  if ((m_type == pushButtonMulti) ||
      (m_type == hotPushButtonMulti) ||
	  (m_type == hotCustomPushButtonMulti)){
      m_bMultiClicked = false;
      HWND hWnd = GetSafeHwnd();
      if ((hWnd != NULL) && ::IsWindow (hWnd))
  ...
  (CString strGroupName)
{
  ...
  if (pGroup == NULL) {
      ...
      ASSERT (pGroup != NULL);
      m_topBtnGroupGroups.SetAt (strGroupName, pGroup);
  }
...
void FooButton::OnLButtonUp (UINT nFlags, CPoint point)
{
  ...
  if ((m_type == checkButton) ||
      ...
      if (rectButton.PtInRect (point))
         if ((m_type != radio) && m_strButtonGroup.IsEmpty())
            check (!isChecked());
         else
            check (true);
  }
   ...
   int& nLeftEdge)
{
  ...
  // Draw frame
  UINT uFrameCtrl = (m_type == checkBox ? DFCS_BUTTONCHECK : DFCS_BUTTONRADIO);
  if (m_bChecked)
     uFrameCtrl |= DFCS_CHECKED;
   ...
   int& nBitmapRightEdge)
{
  ...
  GetClientRect (&rectButton);
  CPoint pt;
  if (m_bText || (m_type == checkBox) || (m_type == radio))
   ...
   int nRightEdge)
{
  ...
  if (!m_bStatic && !m_bHot && (getFocusStyle() != noFocus))
   ...
   LPDRAWITEMSTRUCT lpDrawItemStruct)
{
  ...
  if ((btnType != pushButton) &&
      (btnType != pushButtonDropDown) &&
      (btnType != pushButtonMulti))
     return;

  // Nothing to do if the default border isn't required
  if (getFocusStyle() != defaultFocus)
     return;
...
{
  pGroup = NULL;
  m_topBtnGroupGroups.Lookup (strGroupName, pGroup);
}
