...
void COptionTreeList::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	//by pok, hit test and kill focus;
	//long lHit = m_otOption->HitTest(point);
	//if( m_bItemHit )
	//{
	//	if (lHit == OT_HIT_ATTRIBUTE)
	//	{
	//		COptionTreeItem *otiItem = m_otOption->FindItem(point);
	//		if(otiItem != NULL && otiItem->GetCtrlID() != 1) //1은 언제나 제목
	//		{
	//			CWnd::OnMouseMove(nFlags, point);
	//			return;
	//		}
	...
	//	if(lHit == 2024 || lHit == 2025 ) //엥? 2024 하고 2025는 무엇일까나..
	//	{
	//		CWnd::OnMouseMove(nFlags, point);
	//		return;
	//	}
