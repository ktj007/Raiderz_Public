///////////////////////////////////////////////////////////////////////////////
// 
// CTreeCtrlExM - Multiple selection tree control for MFC
// 
...
BEGIN_MESSAGE_MAP(CTreeCtrlExM, CTreeCtrl)
	//{{AFX_MSG_MAP(CTreeCtrlExM)
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_MOUSEMOVE()
	ON_WM_KEYDOWN()
...
IMPLEMENT_DYNAMIC(CTreeCtrlExM, CTreeCtrl)

BOOL CTreeCtrlExM::Create(DWORD dwStyle, DWORD dwExStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
#if _MFC_VER < 0x0700
...
BOOL CTreeCtrlExM::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
...
void CTreeCtrlExM::OnLButtonDown( UINT nFlags, CPoint point )
{
	UINT nHitFlags = 0;
	...
	if( nHitFlags & TVHT_ONITEM )
	{
		...
		// by pok, check box observer
		if( (nHitFlags & TVHT_ONITEMSTATEICON) && m_pCheckBoxObserver )
			m_pCheckBoxObserver->ObserveCheckBox( hClickedItem );
...
void CTreeCtrlExM::OnLButtonUp( UINT nFlags, CPoint point )
{
	if ( m_bSelectPending )
	{
...
void CTreeCtrlExM::OnMouseMove( UINT nFlags, CPoint point )
{
...
void CTreeCtrlExM::SelectMultiple( HTREEITEM hClickedItem, UINT nFlags, CPoint point )
{
...
void CTreeCtrlExM::OnKeyDown( UINT nChar, UINT nRepCnt, UINT nFlags ) 
{
	CWnd* pWnd = GetParent();
...
void CTreeCtrlExM::OnRButtonDown( UINT nFlags, CPoint point )
{
	UINT nHitFlags = 0;
	HTREEITEM hClickedItem = HitTest( point, &nHitFlags );
...
UINT CTreeCtrlExM::GetSelectedCount() const
{
...
HTREEITEM CTreeCtrlExM::GetNextItem(HTREEITEM hItem, UINT nCode)
{
...
HTREEITEM CTreeCtrlExM::GetFirstSelectedItem()
{
	for ( HTREEITEM hItem = GetRootItem(); hItem!=NULL; hItem = GetNextVisibleItem( hItem ) )
		if ( GetItemState( hItem, TVIS_SELECTED ) & TVIS_SELECTED )
			return hItem;
...
HTREEITEM CTreeCtrlExM::GetNextSelectedItem( HTREEITEM hItem )
{
...
HTREEITEM CTreeCtrlExM::GetPrevSelectedItem( HTREEITEM hItem )
{
...
BOOL CTreeCtrlExM::SelectItemEx(HTREEITEM hItem, BOOL bSelect/*=TRUE*/)
{
...
BOOL CTreeCtrlExM::SelectItems( HTREEITEM hFromItem, HTREEITEM hToItem )
{
...
void CTreeCtrlExM::ClearSelection(BOOL bMultiOnly/*=FALSE*/)
{
//	if ( !bMultiOnly )
//		SelectItem( NULL );
...
BOOL CTreeCtrlExM::OnItemexpanding(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
BOOL CTreeCtrlExM::OnSelchanged(NMHDR* pNMHDR, LRESULT* pResult)
{
...
BOOL CTreeCtrlExM::OnSetfocus(NMHDR* pNMHDR, LRESULT* pResult) 
{
	Invalidate();
	*pResult = 0;
	return FALSE;
...
BOOL CTreeCtrlExM::OnKillfocus(NMHDR* pNMHDR, LRESULT* pResult) 
{
	Invalidate();
	*pResult = 0;
	return FALSE;
...
void CTreeCtrlExM::OnLButtonDblClk(UINT nFlags, CPoint point)
{
...
void CTreeCtrlExM::OnTimer(UINT nIDEvent)
{
...
HTREEITEM CTreeCtrlExM::ItemFromData(DWORD dwData, HTREEITEM hStartAtItem/*=NULL*/) const
{
