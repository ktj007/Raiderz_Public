...
//    apRect->right = apRect->left + iDefTreeIndent - TREE_BOX_MARGIN;
	apRect->right = apRect->left + nBoxSize;
    apRect->top = iMargin;
//    apRect->bottom = apRect->top + iDefTreeIndent - (2*TREE_BOX_MARGIN) ;
	apRect->bottom = apRect->top + nBoxSize;
                                    ...
                                    CRect aRect)    // coordinates of bounding cell rectangle
{
	...
	LOGBRUSH LogBrush;
	LogBrush.lbColor = m_pTreeColumn->GetTreeLineColor();
	LogBrush.lbStyle = PS_SOLID;
    CPen psPen( PS_COSMETIC | PS_ALTERNATE, 1, &LogBrush );

    CPen* pOldPen = apDC->SelectObject(&psPen);

	CPen psSolidPen(PS_SOLID, 1, m_pTreeColumn->GetTreeLineColor() );
    TREE_IMAGE TreeImage = TREE_IMAGE_DOCUMENT;
