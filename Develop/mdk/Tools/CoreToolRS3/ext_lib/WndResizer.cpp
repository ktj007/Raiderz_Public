...
static CMap<HWND, HWND, CWndResizer *, CWndResizer *> WindResizer_WndData;
...
void CWndResizer::OnMouseMove(UINT nFlags, CPoint point)
{
  if (pCaptured != NULL )
  {
    if ((nFlags & MK_LBUTTON) <= 0)
    {
      ...
      if (pCaptured->PtInRect( point ) == FALSE || hWndFromPoint != m_pHookedWnd->m_hWnd)
      {
        ::ReleaseCapture();      
        pCaptured = NULL;
        HCURSOR hCur = ::SetCursor(hOldCursor);
        ::DestroyCursor( hCur );
        hOldCursor = NULL;
      }
    }
  ...
  if (pCaptured == NULL )
  {
    pCaptured = FindSplitterFromPoint(&root, point);
    if (pCaptured != NULL)
    {
      m_pHookedWnd->SetCapture();
      LPCTSTR cursor = NULL;
      CSplitContainer * pSplitContainer = (CSplitContainer *)pCaptured->Parent;
      ...
      HCURSOR hOldCursor = ::SetCursor(hCur);
    }
  }

  if (pCaptured != NULL && (nFlags & MK_LBUTTON) > 0)
  {
	m_pHookedWnd->SendMessage(WM_USER_WND_RESIZER_ENABLE);
    CSplitContainer * pSplitterContainer = (CSplitContainer *) pCaptured->Parent;
    
    if (pSplitterContainer->m_Orientation == CWndResizer::SPLIT_CONTAINER_H)
    {
      pSplitterContainer->SetSplitterPosition( point.x - splitterOffset);
    }
    else
    {
      pSplitterContainer->SetSplitterPosition( point.y - splitterOffset);
    }
    UpdateSplitterOffset(point);
...
void CWndResizer::OnLButtonUp(UINT nFlags, CPoint point)
{
  ...
  m_pHookedWnd->SendMessage(WM_USER_WND_RESIZER_DISABLE);
}
void CWndResizer::UpdateSplitterOffset(CPoint ptCurr)
{
  ...
  if (((CSplitContainer *)pCaptured->Parent)->m_Orientation == CWndResizer::SPLIT_CONTAINER_H)
  {
    if ( ptCurr.x  < pCaptured->left)
    {
      splitterOffset = 0;
    }
    else if ( ptCurr.x > pCaptured->right)
    {
      splitterOffset = pCaptured->Width();
    }
    else
    {
      splitterOffset = ptCurr.x - pCaptured->left;
    }
  }
  else
  {
    if ( ptCurr.y  < pCaptured->top)
    {
      splitterOffset = 0;
    }
    else if ( ptCurr.y > pCaptured->bottom)
    {
      splitterOffset = pCaptured->Height();
    }
    else
    {
      splitterOffset = ptCurr.y - pCaptured->top;
    }    
  }
}
...
BOOL CWndResizer::Hook(CWnd * pParent)
{
  ...
  WindResizer_WndData.SetAt(m_pHookedWnd->m_hWnd, this);
...
BOOL CWndResizer::Unhook()
{
  ...
  WindResizer_WndData.RemoveKey(m_pHookedWnd->m_hWnd); 
...
LRESULT CALLBACK CWndResizer::WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  WindResizer_WndData.Lookup(hWnd, pResizer);
