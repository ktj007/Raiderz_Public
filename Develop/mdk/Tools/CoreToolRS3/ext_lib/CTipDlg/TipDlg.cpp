...
static const TCHAR szIntStartup[] = _T("Ver0001");
	...
	: CDialog(IDD_TIP, pParent)
{
	...
	CSplitPath split (p);
    CString sPath ;
    sPath = split.getdrive() + CString(split.getdir()) + "Tips.tip" ;
	
	m_pStream = fopen(sPath, "r");
	if (m_pStream == NULL) 
	{
		m_strTip = "팁 파일이 존재하지 않습니다";
		return;
	} 
	...
	{
		AfxMessageBox("팁 파일을 읽는데 문제가 발생했습니다.");
	}
	else 
	{
...
void CTipDlg::DoPaint (CDC *pdc, BOOL updatetext)
{
	...
	CFont font, fonttext ;
	BOOL fok = font.CreateFont(20, 0, 0, 0, FW_BOLD, 0, 0, 0,
					  DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
					  CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
					  DEFAULT_PITCH|FF_SWISS, "굴림");

	fok = fonttext.CreateFont(12, 0, 0, 0, FW_NORMAL, 0, 0, 0,
					  DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
					  CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
					  DEFAULT_PITCH|FF_SWISS, "굴림");
