...
#define DEFAULT_SHOW_DELAY		500		// Default show delay (ms)
#define TIMER_HIDE				100		// Hide timer (ms) //by pok, 현재는 자기 윈도우 영역이 아니면 바로 지운다.
...
//     
/////////////////////////////////////////////////////////////////////
void CXInfoTip::Show(CString szText)
{
	m_szText = szText;
...
void CXInfoTip::OnTimer( UINT nIDEvent ) 
{
	...
	case timerShow:
		...
		if( m_ptOrigin == ptCursor )
		{
			...
			SetWindowPos(&wndTop, m_ptOrigin.x - CX_ROUNDED, m_ptOrigin.y - WindowSize.cy + CY_ROUNDED, WindowSize.cx, WindowSize.cy, SWP_NOACTIVATE | SWP_SHOWWINDOW);
		}
...
void CXInfoTip::RelayEvent(LPMSG lpMsg)
{
	...
	case WM_MOUSEMOVE:
		...
		{
			// Find the tool
			pWindow = CWnd::WindowFromPoint(point);
			if (pWindow != NULL)
			{
				if (m_ToolMap.Lookup(pWindow->m_hWnd, Info))
				{
					...
					// by pok, Hide the tooltip, 영역이 아니면 바로바로 툴팁 없애기
					ShowWindow(SW_HIDE);
			}
		}
