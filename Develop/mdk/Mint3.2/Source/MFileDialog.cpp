...
{
	SetRect(MRECT(FILEDIALOG_X, FILEDIALOG_Y, FILEDIALOG_W, FILEDIALOG_H));

	MRECT r = GetClientRect();

	m_pOK = new MButton( "", this, this);
	m_pOK->SetRect(MRECT(OK_X, OK_Y, BUTTON_W, BUTTON_H));
	m_pOK->SetLabelAccelerator();

	m_pOK->SetAnchor(MAnchorStyle(false, true, false, true));

	m_pCancel = new MButton( "", this, this);
	m_pCancel->SetRect(MRECT(CANCEL_X, CANCEL_Y, BUTTON_W, BUTTON_H));
	m_pCancel->SetAccelerator(27);

	m_pCancel->SetAnchor(MAnchorStyle(false, true, false, true));


	m_pFileBox = new MFileBox( "", this, this);
	m_pFileBox->SetRect(MRECT(FILEBOX_X, FILEBOX_Y, FILEBOX_W, FILEBOX_H));
	m_pFileBox->SetAnchor(MAnchorStyle(true, true, true, true));

	m_pFileName = new MEdit( "", this, this);
	m_pFileName->SetRect(MRECT(EDIT_X, EDIT_Y, EDIT_W, EDIT_H));
	//m_pFileName->SetTextColor(MCOLOR(0, 255, 0));
	m_pFileName->SetAnchor(MAnchorStyle(true, true, false, true));

	m_pMsgBox = new MMsgBox( "", this, this, MT_YESNO);
