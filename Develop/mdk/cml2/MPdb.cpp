...
typedef DWORD	(WINAPI *fn_sym_LoadModule)				( HANDLE hProcess, HANDLE hFile, PCSTR pImageName, LPSTR pModuleName, DWORD dwBaseOfDll, DWORD dwSizeOfDll);
...
BOOL CALLBACK EnumLoadedModulesCallback(PCSTR pModuleName, ULONG ulModuleBase,  ULONG ulModuleSize,  PVOID pUserContext)
{
...
//			::MessageBox(NULL,"SymGetSymFromAddr failed","error",MB_OK);
			...
			sprintf(t_temp,"SymGetSymFromAddr error %d\n",GetLastError());str += t_temp;			
		}
		...
		if (g_pfnSymGetModuleInfo(hProcess, stk.AddrPC.Offset, &mi))
		{
			...
			sprintf(t_temp,"SymGetModuleInfo error %d\n",GetLastError());str += t_temp;			
		}
		...
		if (g_pfnSymGetLineFromAddr)
		{
			if (g_pfnSymGetLineFromAddr( hProcess, stk.AddrPC.Offset, &dwDisplacement, &imageLine))
			{
				...
				sprintf(t_temp,"SymGetLineFromAddr error %d\n",GetLastError());str += t_temp;			
			}
		}
