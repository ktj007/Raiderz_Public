...
MUID MUIDRandomGenerator::Generate()
{
	LARGE_INTEGER nPerfCounter;
	QueryPerformanceCounter( &nPerfCounter );

	int ran = rand();
	MUID ret(nPerfCounter.LowPart, (nPerfCounter.HighPart << 16 | (m_nSeed << 8 | ran) | (nPerfCounter.LowPart & 0xFFFF)));
	return ret;
}