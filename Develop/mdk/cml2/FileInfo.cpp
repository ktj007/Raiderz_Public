...
BOOL GetLastDirectoryUpdate(const char* pDirectoryName, FILETIME *ft)
{
	HANDLE hFile;
	hFile = CreateFile(pDirectoryName,GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL|FILE_FLAG_BACKUP_SEMANTICS,NULL);
	if(hFile==INVALID_HANDLE_VALUE)
	{
		return FALSE;
	}
...
void TrimFilePathName( char* szFileName)
{
	// 맨 앞의 "/", "\", "./", ".\" 삭제
	while ( true)
	{
		...
		else if ( szFileName[ 0] == '/'  ||  szFileName[ 0] == '\\')
			memcpy( szFileName, szFileName + 1, strlen( szFileName));

		else if ( (szFileName[ 0] == '.'  &&  szFileName[ 1] == '/')  ||  (szFileName[ 0] == '.'  &&  szFileName[ 1] == '\\'))
			memcpy( szFileName, szFileName + 2, strlen( szFileName) - 1);
	...
	// "\"를 "/"로 변경
	size_t _size = strlen( szFileName);
...
void GetMergedPath( char* szOutPath, const char* szBasePath, const char* szRelPath)
{
	szOutPath[ 0] = 0;

	char szSrcPath[ _MAX_PATH];
	strcpy_s( szSrcPath, szBasePath);
	TrimFilePathName( szSrcPath);

	char szTarPath[ _MAX_PATH];
	strcpy_s( szTarPath, szRelPath);
	TrimFilePathName( szTarPath);


	while ( true)
	{
		if ( szSrcPath[ 0] == 0  ||  szTarPath[ 0] == 0)		break;
		if ( szTarPath[ 0] == '.'  &&  szTarPath[ 1] == '.'  &&  szTarPath[ 2] == '/')
		{
			while ( szSrcPath[ 0])
			{
				char* c = szSrcPath + ( strlen( szSrcPath) - 1);
				if ( *c == '/'  ||  *c == '\\')
				{
					*c = 0;
					break;
			...
			memcpy( szTarPath, szTarPath + 3, strlen( szTarPath) - 2);
			continue;
		}
	...
	strcpy_s( szOutPath, _MAX_PATH, szSrcPath);
	strcat_s( szOutPath, _MAX_PATH, "/");
	strcat_s( szOutPath, _MAX_PATH, szTarPath);
}
...
void GetPureFilenameExt(char *pPureFilenameExt,const char *pFilename)
{
	char drive[_MAX_DRIVE],dir[_MAX_DIR],ext[_MAX_EXT];
	_splitpath(pFilename,drive,dir,pPureFilenameExt,ext);
	strcat(pPureFilenameExt,ext);
}
