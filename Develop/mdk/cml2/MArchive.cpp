...
vec3 MAITEM::GetVectorValue(void)
{
...
void MArchive::Add(const char* szName, vec3& v)
{
	Add(szName, MAT_VECTOR, &v, sizeof(vec3));
}
...
bool MArchive::GetFirst(const char* szName, vec3* v)
{
	MAITEM* pItem = GetFirst(szName);
	if(pItem==NULL) return false;
...
bool MArchive::GetNext(const char* szName, vec3* v)
{
	MAITEM* pItem = GetNext(szName);
	if(pItem==NULL) return false;
	*v = pItem->GetVectorValue();
