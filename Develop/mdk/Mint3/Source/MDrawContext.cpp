...
#define		MAX_WIDGET_LINE_LENGTH	1024
	...
	for(int i=0; i<nStrLen; i++)
	{
		...
		if(c>127)
		{
			...
			for_index = i;				// 한글에서 마지막 글자 하나를 찍지 못하는 벅스 수정
		}
...
int MDrawContext::GetTextMultiLine(vector<string> *out_pvecStr, const char* szText, int nStrWidth, bool bSkipColor)
{
	...
	char chPreColor[3]		= {0, };							// 이전 줄 색깔
	char strTemp[1024]		= {0, };							// 임시 저장소
	char* pstrTemp			= strTemp;
	int nstrCount			= strlen(szText);					// 문자열 길이
	int nPreIndex			= 0;								// 이전 문자열 인덱스, 색깔까지 고려하다 보니... for문 마지막에 쓰이게 되었다.
	...
	for(int i = 0; i < nstrCount; i++)
	{
		...
		*pstrTemp			= szText[i];
		...
		{
			// nPreIndex에서 1를 빼는건... 현재 글자가 오버를 했으므로
			// 이전 글자까지만 가져오기 위한것이다.
