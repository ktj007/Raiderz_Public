...
IMPLEMENT_NULL_LOOK(MEdit, MNullEditLook)
	...
	case MWM_KEYDOWN:
		{
			...
			if(pListener!=NULL) pListener->OnCommand(this, MEVENT_EDIT_KEYDOWN);
	...
	case MWM_CHAR:
		{
			...
			MListener* pListener = GetListener();
			if(pListener!=NULL) pListener->OnCommand(this, MEVENT_EDIT_CHAR);
	...
	case MWM_LBUTTONDOWN:
		// 마우스 클릭으로 캐럿 포지셔닝
		{
			...
			if(r.InPoint(pEvent->Pos) == true)
			{
				...
				if(nPos < 0) 
					return true;
					//return false;			// false를 하면 우선순위 렌더링에서 뒤로 물러나게 된다.
				m_nCaretPos = nPos;
				bMyEvent = true;
				return true;
			}
...
bool MEdit::InputFilterKey(int nKey)
{
	if(nKey==VK_DELETE){
		...
		if(m_nCurrentHistory==m_History.end()){
			...
			if(m_nCurrentHistory!=m_History.begin())
				m_nCurrentHistory--;
		}
		...
		MListener* pListener = GetListener();
		if(pListener!=NULL) return pListener->OnCommand(this, MEVENT_EDIT_ENTER);
		return false;
	}
	else if(nKey==VK_TAB){
		if (GetTabHandler()) {
...
bool MEdit::InputFilterChar(int nKey)
{
	if(nKey==VK_BACK){
		...
		*/
		...
		//ReleaseFocus();
		MListener* pListener = GetListener();
		if(pListener!=NULL) pListener->OnCommand(this, MEVENT_EDIT_ESC);
		return true;
	}
...
int MEdit::GetPosByScreen(int x)
{
	...
	if(for_index==0) return GetStartPos();
...
void MEdit::SetNumber( bool bNumber)
{
	if ( bNumber == true)
	{
		int num = atoi( GetText());
		char szTmp[ 256];
		_itoa( num, szTmp, 10);
...
void MEditLook::OnFrameDraw(MEdit* pEdit, MDrawContext* pDC)
{
	...
	pDC->SetColor(MColorTable::MEDIT_PLANE);
	pDC->FillRectangle(r);
	pDC->SetColor(MColorTable::MEDIT_OUTLINE);
	pDC->Rectangle(r);
}
...
#define BUFFERSIZE	1024
	...
	*/
	...
	pDC->SetColor(MColorTable::MEDIT_TEXT);
