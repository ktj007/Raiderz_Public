...
IMPLEMENT_NULL_LOOK(MTextArea, MNullTextAreaLook)
...
bool MTextArea::OnCommand(MWidget* pWidget, MEventMsg nMsg, const char* szArgs)
{
...
bool MTextArea::OnLButtonDown(MPOINT pos)
{
	...
	for(int i=GetStartLine();i<GetLineCount();i++)
	{
		...
		if(pos.y <= y + nLine*GetLineHeight() ||	// 이 줄을 클릭했거나
			...
			char szLink[ 512];
			bool bFind = false;
			for ( int j = 0;  j < ((int)m_CurrentLine->text.size() - 1); j++)
			{
				if ( (j > m_CaretPos.x) || (bFind == true))
					break;

				if ( (szText[j] == '%') && (szText[j+1] == '%'))
				{
					int nIndex = 0;

					for ( int k = (j+2);  k < ((int)m_CurrentLine->text.size() - 1); k++)
					{
						if ( (szText[k] == '%') && (szText[k+1] == '%'))
						{
							if ( k > m_CaretPos.x)
								bFind = true;
			...
			if ( bFind)
			{
				m_LinkedTextArgs.Set( szLink, szLink);
				m_EventHandler.OnCommand(this, MEVENT_TEXTAREA_LINKEDTEXT);
			}
...
bool MTextArea::InputFilterChar(int nKey)
{
	if(nKey==VK_BACK){
		...
		if(pListener!=NULL) pListener->OnCommand(this, MEVENT_TEXTAREA_ESC_VALUE);
		return true;
	}
 ...
 : MWidget(szName, pParent, pListener) , m_TextOffset(0, 0)
{
	...
	m_TextColor = MColorTable::TEXTAREA_TEXT; 
...
// 	return i->text.c_str();
	return m_Lines[nLine].text.c_str();
}
...
void MTextAreaLook::OnFrameDraw(MTextArea* pTextArea, MDrawContext* pDC)
{
	...
	pDC->SetColor(MColorTable::TEXTAREA_PLANE);
	pDC->FillRectangle(r);
	pDC->SetColor(MColorTable::TEXTAREA_OUTLINE);
	pDC->Rectangle(r);
}
...
void MTextArea::glueClear()
{
	this->Clear();
}

void MTextArea::glueAddText(const char* szText)
{
	if (szText == NULL) return;
	this->AddText(szText);
}

void MTextArea::glueAddColorText(const char *szText, unsigned int color)
{
	if (szText == NULL)
	{
		return;
	}
	AddText(szText, color);
}

// 라인수와 font height를 곱해서 height사이즈를 넘긴다.
int MTextArea::glueGetMaxHeight()
{
	MFont *pFont=GetFont();
	MRECT r=GetClientRect();
	...
	//크기로 바꿔서 리턴.
	int size = nTotalLine * GetLineHeight();
