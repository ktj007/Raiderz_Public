...
// XRealSoundLoader
XRealSoundLoader::XRealSoundLoader()
{
}
...
// LoadSamples
bool XRealSoundLoader::LoadSamples( const char* szFileName)
{
	MXml xml;
	...
	for ( pElement;  pElement != NULL;  pElement = pElement->NextSiblingElement())
	{
		...
		else if ( _stricmp( pElement->Value(), "SOUND") == 0)
		{
			...
			XSample* pSample = new XSample( _info);
			if ( global.sound->AddSample( strName.c_str(), pSample) == false)
			{
#ifndef _PUBLISH
