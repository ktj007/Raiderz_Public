...
bool ZChat::Input(char* szMsg)
{
	...
	if (szMsg[0] == '/')
	{
		if (strlen(szMsg) >= 2)
		{
			...
			{
				XChatCmdFlag nCurrFlag = CCF_NONE;
				...
				int nCmdInputFlag = XChatCmdManager::CIF_NORMAL;
				...
				if (m_CmdManager.DoCommand(&szMsg[1], nCurrFlag, XChatCmdManager::CmdInputFlag(nCmdInputFlag)))
				{
					return true;
				}
				else
...
void ZChat::FilterWhisperKey(MWidget* pWidget)
{
	...
	if ((!stricmp(text, "/r ")) || (!stricmp(text, "/ㄱ ")))
	{
		...
		XChatCmd* pWhisperCmd = GetCmdManager()->GetCommandByID(CCMD_ID_WHISPER);
		if (pWhisperCmd)
		{
