...
bool GItemRemover::RemoveForDBTask(const MUID& nPlayerUID, vector<REMOVE_ITEM>& vecRemoveItem, eROUTE route)
{
...
bool GItemRemover::RemoveForDBTask( const MUID& nPlayerUID, const REMOVE_ITEM& removeItem, eROUTE route )
{
	vector<REMOVE_ITEM> vecRemoveItem;
	vecRemoveItem.push_back(removeItem);

	return RemoveForDBTask(nPlayerUID, vecRemoveItem, route);
}
