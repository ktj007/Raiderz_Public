...
MCommand* GEntityPlayer::CreateFieldInCmd()
{
	...
	MakeTDCacheInfo(td_player_info, td_player_feature_Tattoo);

	TD_PLAYER_FEATURE_TATTOO* ptd_player_featureTattoo = NULL;
	if (td_player_feature_Tattoo.IsValid())
	{
...
void GEntityPlayer::OnSectorChanged(const MPoint& oldSectorPt)
{
	...
	delta_selector->GetDeltaNewHugeSectors(sectors, oldSectorPt, GetSectorPoint());

	GFieldGridRouter sector_router;
	sector_router.SendToMe(this, sectors);
...
void GEntityPlayer::MakeTDCacheInfo(TD_UPDATE_CACHE_PLAYER& cache, TD_PLAYER_FEATURE_TATTOO& TattooCache)
{
	GPlayerTDCacheInfoBuilder tdCacheInfoBuilder;
	tdCacheInfoBuilder.Build(this, cache, TattooCache);
}
...
void GEntityPlayer::RouteInEntity( const vector<MUID>& vecPlayers )
{
	...
	TD_PLAYER_FEATURE_TATTOO* ptd_player_feature_Tattoo = NULL;
	if (td_player_feature_Tattoo.IsValid())
	{
...
void GEntityPlayer::CalcNewCells( vector<GFieldGrid::Sector*>& vecNewSectors, MPoint ptOldSec, MPoint ptNewSec ) const 
{
	__super::CalcNewCells(vecNewSectors, ptOldSec, ptNewSec);

	vector<GFieldGrid::Sector*> vecNewHugeCells;
	GFieldGrid::DeltaSelector* delta_selector = 
		m_pField->GetGrid().GetDeltaSelector();

	delta_selector->GetDeltaNewHugeSectors(vecNewHugeCells, ptOldSec, ptNewSec);
	vecNewSectors.insert(vecNewSectors.end(), vecNewHugeCells.begin(), vecNewHugeCells.end());
}

void GEntityPlayer::OnSectorChanged(const vector<GFieldGrid::Sector*>& vecNewSectors, const vector<GFieldGrid::Sector*>& vecOldSectors)
{
	VALID(m_pField);
	...
	GVectorMUID vecNewNPC;
	entity_selector->GetNPCs(vecNewNPC.Vector(), vecNewSectors);
	GVectorMUID vecOldNPC;
	entity_selector->GetNPCs(vecOldNPC.Vector(), vecOldSectors);

	GetNPCIconSender().RemoveNPCIconData(vecOldNPC.Vector());
	GetNPCIconSender().SendByPlayerIn(vecNewNPC.Vector(), vecOldSectors.empty());
}

float GEntityPlayer::GetMoveSpeed() 
{
	if (GetRide().IsMounted())
