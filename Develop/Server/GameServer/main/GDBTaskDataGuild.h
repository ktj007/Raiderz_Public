...
public :
	...
	GDBT_GUILD(const AID nAID, const MUID& uidPlayer, const CID nCID, int nCharPtm, int nDeltaCharPtm, const int nMoney, const int nDeltaMoney, uint8 nLevel, const int nCODE, const int64 nGID, const wstring strName = L"") :
...
public :
	...
	GDBT_GUILD_CHANGE_MASTER(const MUID uidPreMaster, const CID nCID) : m_uidPreMaster(uidPreMaster), m_nPreMasterCID(nCID) {}

	void SetNewMaster(const AID nAID, const CID nCID, const int nCODE, const int64 nGID)
	{
...
public:
	...
	GDBT_GUILD_UPDATE_STORAGEMONEY(TYPE nType, AID nAID, MUID uidPlayer, CID nCID, int nMoney, int64 nGID, int nStorageMoney, int nModMoney) : m_nType(nType), m_nAID(nAID), m_uidPlayer(uidPlayer), m_nCID(nCID), m_nMoney(nMoney), m_nGID(nGID), m_nStorageMoney(nStorageMoney), m_nModMoney(nModMoney) {}
