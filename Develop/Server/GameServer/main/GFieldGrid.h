...
// 	};
	...
	public:
		...
		// aabb나 radius또한 마찬가지로 계산 결과가 광범위한 섹터를 요구하더라도 5x5이상 하지 않는다.
...
//		void GetAllNeighborSectorsExceptThis( vector<Sector*>& vecSectors, MPoint ptPoint, int nSize ) const;
	...
	public:
		...
		void GetEntities( vector<MUID>& vecEntities, ENTITY_TYPEID type, const vector<Sector*>& vecSectors, MUID uidExcept = MUID::ZERO ) const;	//todo 제거예정
		void GetNPCs( vector<MUID>& vecEntities, const vector<Sector*>& vecSectors, MUID uidExcept = MUID::ZERO) const;//todo 제거예정
