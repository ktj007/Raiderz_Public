...
bool GTimeDelayHitChecker::CheckTime( float fElapsedTime, GEntityActor* pOwner, GTalentInfo* pTalentInfo )
{
	...
	vector<GEntityActor*> qEffecteeList = effecteeTargetSelector.Select(desc);
...
void GTalentHitter_Projectile::MakeEnemyList(GField* pField)
{
	...
	GVectorMUID vecActors;
	grid_selector->GetActors(vecActors.Vector(), pOwner->GetPos());
	for each (const MUID& uid in vecActors.Vector())
	{
		GEntityActor* pTarget = pField->FindActor(uid);
		if (!pTarget) 
			continue;	// 유효하지 않은 목표
		GRelationChecker relationChecker;
		if (!relationChecker.IsEnemy(pOwner, pTarget))
			continue;	// 적이 아님
		m_vecEnemyList.push_back(uid);
	}
}
