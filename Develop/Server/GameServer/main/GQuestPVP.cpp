...
void GQuestPVP::RewardEvent(GQPEEndInfo* pEndInfo)
{
	...
	vector<GEntityPlayer*> vecWinnerPlayer = CollectTeam(pEndInfo->m_nWinnerTeam);
...
vector<GEntityPlayer*> GQuestPVP::CollectTeam(QUESTPVP_TEAM nTeam)
{
	const GFieldInfo* pInfo = m_pField->GetInfo();
	if (NULL == pInfo) return vector<GEntityPlayer*>();
	GFieldGrid::EntitySelector* entity_selector = m_pField->GetGrid().GetEntitySelector();
	if (NULL == entity_selector) return vector<GEntityPlayer*>();
	if (true == pInfo->m_PvPAreas.empty())	return vector<GEntityPlayer*>();	

	set<MUID> setPlayerUID;
	for each (const PVP_AREA_INFO& pvpAreaInfo in pInfo->m_PvPAreas)
	{
		vector<MUID> vecPlayerUID;
		entity_selector->GetPlayers(vecPlayerUID, pvpAreaInfo.rcArea);

		for each (const MUID& nPlayerUID in vecPlayerUID)
		{
			setPlayerUID.insert(nPlayerUID);
		}		
	}
...
{
	vector<GEntityPlayer*> vecPlayer = CollectTeam(nTeam);
...
{
	vector<GEntityPlayer*> vecTeamOnePlayer = CollectTeam(QT_ONE);
	vector<GEntityPlayer*> vecTeamTwoPlayer = CollectTeam(QT_TWO);
