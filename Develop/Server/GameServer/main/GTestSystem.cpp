...
void GTestSystem::ClientRenderHitCapsule( GEntityActor* pOwner, const MCapsule& capsule )
{
	...
	pField->GetGrid().GetEntitySelector()->GetPlayers(vecPlayers, pOwner->GetPos());

	for each (MUID each in vecPlayers)
	{
		GEntityPlayer* pPlayer = pField->FindPlayer(each);
		if (!pPlayer)
			continue;

		if (m_mapSubscriptors_ActorHitCapsule.find(pPlayer->GetUID()) == m_mapSubscriptors_ActorHitCapsule.end())
			continue; // 구독하지 않음

		RouteToClientRenderCapsule(pPlayer->GetUID(), pOwner, capsule);
	}
}
...
bool GTestSystem::RouteAllNearActorCapsule( MUID uidPlayer )
{
	...
	pField->GetGrid().GetEntitySelector()->GetNPCs(vecActors, pPlayer->GetPos());

	vector<TD_DEBUG_POS> vecTDPositions;
	for each (MUID each in vecActors)
	{
		GEntityNPC* pActor = pField->FindNPC(each);
		if (!pActor)
			continue;

		ClientRenderActorCapsule(uidPlayer, pActor);
	}
...
bool GTestSystem::RouteActorPos( MUID uidPlayer )
{
	...
	vector<MUID> vecActors;
	pField->GetGrid().GetEntitySelector()->GetActors(vecActors, pPlayer->GetPos());

	vector<TD_DEBUG_POS> vecTDPositions;
	for each (MUID each in vecActors)
	{
		GEntityActor* pActor = pField->FindActor(each);
		if (!pActor)
			continue;
		...
		infoDebug.uidOwner = each;

		if (pActor->IsNPC())
		{
			GEntityNPC* pNPC = ToEntityNPC(pActor);
			GEntityActor* pTarget = pNPC->GetTargetActor();

			if (pTarget)
				infoDebug.uidTarget = pTarget->GetUID();
		}

		vecTDPositions.push_back(infoDebug);
	}
