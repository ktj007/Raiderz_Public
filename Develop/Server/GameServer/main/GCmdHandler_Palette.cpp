...
MCommandResult GCmdHandler_Palette::OnRequestSelect(MCommand* pCmd, MCommandHandler* pHandler)
{
	...
	if (pCmd->GetParameter(&nNum,			1, MPT_UCHAR)==false) return CR_FALSE;
...
MCommandResult GCmdHandler_Palette::OnRequestPutUp(MCommand* pCmd, MCommandHandler* pHandler)
{
	...
	if (pCmd->GetParameter(&eSkillSetType,		0, MPT_INT)==false) return CR_FALSE;
	if (pCmd->GetParameter(&nNum,				1, MPT_UCHAR)==false) return CR_FALSE;
	if (pCmd->GetParameter(&nSlot,				2, MPT_UCHAR)==false) return CR_FALSE;
	if (pCmd->GetParameter(&nType,				3, MPT_UCHAR)==false) return CR_FALSE;
	if (pCmd->GetParameter(&nItemIDorTalentID,	4, MPT_INT)==false) return CR_FALSE;
...
MCommandResult GCmdHandler_Palette::OnRequestChange(MCommand* pCmd, MCommandHandler* pHandler)
{
	...
	PALETTE_SLOT nSlot2;
	if (pCmd->GetParameter(&eSkillSetType,	0, MPT_INT) == false) return CR_FALSE;
	if (pCmd->GetParameter(&nNum1,			1, MPT_UCHAR)==false) return CR_FALSE;
	if (pCmd->GetParameter(&nSlot1,			2, MPT_UCHAR)==false) return CR_FALSE;
	if (pCmd->GetParameter(&nNum2,			3, MPT_UCHAR)==false) return CR_FALSE;
	if (pCmd->GetParameter(&nSlot2,			4, MPT_UCHAR)==false) return CR_FALSE;
