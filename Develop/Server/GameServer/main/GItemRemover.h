...
public:
	...
	bool RemoveForDBTask(const MUID& nPlayerUID, vector<REMOVE_ITEM>& vecRemoveItem,eROUTE route = ROUTE);
	bool RemoveForDBTask(const MUID& nPlayerUID, const REMOVE_ITEM& removeItem,eROUTE route = ROUTE);
