...
SUITE(PlayerTDCacheInfoBuilder)
{
	...
	public:
		FPlayerTDCacheInfoBuilder()
		{
			...
			pPlayerInfo->nEyeColor = 11;
			pPlayerInfo->nMakeUp = 7;
			pPlayerInfo->nTattooType = 3;
			pPlayerInfo->nTattooPosX = 95;
			pPlayerInfo->nTattooPosY = 4;
			pPlayerInfo->nTattooScale = 3;
	...
	TEST_FIXTURE(FPlayerTDCacheInfoBuilder, TestPlayerTDCacheInfoBuilder_BuildFeature)
	{
		...
		TD_UPDATE_CACHE_PLAYER td;
		TD_PLAYER_FEATURE_TATTOO tdFeatureTattoo;

		m_Builder.BuildFeature(m_pPlayer, td, tdFeatureTattoo);
		...
		CHECK_EQUAL(td.Feature.nEyeColor,	pPlayerInfo->nEyeColor);
		CHECK_EQUAL(td.Feature.nEyeColor,	pPlayerInfo->nEyeColor);
		CHECK_EQUAL(td.Feature.nMakeUp,		pPlayerInfo->nMakeUp);

		CHECK_EQUAL(tdFeatureTattoo.nTattooType,	pPlayerInfo->nTattooType);
		CHECK_EQUAL(tdFeatureTattoo.nTattooPosX,	pPlayerInfo->nTattooPosX);
		CHECK_EQUAL(tdFeatureTattoo.nTattooPosY,	pPlayerInfo->nTattooPosY);
		CHECK_EQUAL(tdFeatureTattoo.nTattooScale,	pPlayerInfo->nTattooScale);
