...
void FBasePlayer::MakeCombat( GEntityPlayer* pEntityPlayer, GEntityNPC* pOpponent )
{
	...
	pEntityPlayer->GetPlayerCombatChecker().Update(0.0f);
}

void FBasePlayer::MakePeace( GEntityPlayer* pEntityPlayer, GEntityNPC* pOpponent )
{
	VALID(pEntityPlayer);
	VALID(pOpponent);
	pOpponent->doMakePeaceForce();
	pEntityPlayer->doMakePeace();
}
