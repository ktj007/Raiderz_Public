...
void LDBTaskPmangAccountInsert::OnExecute( mdb::MDatabase& rfDB )
{
	...
	m_Data.m_nGSN= rs.FieldW(L"ACCN_SN").AsInt64();
	m_Data.m_nCONN_SN= rs.FieldW(L"CONN_SN").AsInt64();
	if (0 == m_Data.m_nGSN)
	{
		SetTaskFail();
		return;
...
void LDBTaskPmangAccountInsert::Completer::Do()
{
	...
	{
		AddNewPlayer();
		LCmdRouter_Player::PostPmangAddPlayerReq(m_Data.uidPlayer, m_Data.m_nGSN, m_Data.strUserID, &m_Data.pmangUserData);
	}
	else
	{
		SCmdRouter_Login cmdRouter(gsys.pCommandCenter);
		cmdRouter.ResponsePmangLogin(m_Data.uidPlayer, CR_FAIL_LOGIN_NOT_EXIST_USER);
	}
}
