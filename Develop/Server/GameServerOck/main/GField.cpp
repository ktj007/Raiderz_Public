...
void GField::UpdateUsableSensor(const SENSOR_INFO* pSensor) const
{
	...
	class DoUpdateUsableSensor : public GFieldGrid::Sector::IVisitor
	{
	public:
		DoUpdateUsableSensor(const GField& field) : m_field(field) {}
	private:
		void OnVisit(const MUID& uid)
		{
			...
			player->UpdateUsableSensor(true);
		}
	private:
		const GField& m_field;
	};
	grid_selector->VisitByPos(DoUpdateUsableSensor(*this), ETID_PLAYER, pSensor->vPoint, 1);
}
