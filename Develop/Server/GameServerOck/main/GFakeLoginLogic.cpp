...
bool GFakeLoginLogic::CheckInsertCharacter_Parameter(MUID uidPlayer, TD_INSERT_CHARACTER_INFO* pInsertCharInfo)
{
	...
	if (false == infoChecker.CheckAllParam_InsertCharacter(pInsertCharInfo->szName, sex
													, pInsertCharInfo->nHair, pInsertCharInfo->nFace
													, pInsertCharInfo->nHairColor, pInsertCharInfo->nSkinColor, pInsertCharInfo->nVoice))
	{
		dlog("Invalid Param - InsertCharacter(Name:%s, Sex:%d, Hair:%d, Face:%d, HairColor:%d, SkinColor:%d, Voice:%d)\n"
			, pInsertCharInfo->szName, (int)pInsertCharInfo->nSex, (int)pInsertCharInfo->nHair, (int)pInsertCharInfo->nFace
			, (int)pInsertCharInfo->nHairColor, (int)pInsertCharInfo->nSkinColor, (int)pInsertCharInfo->nVoice);
...
bool GFakeLoginLogic::ApplyInsertCharacter(MUID uidPlayer, AID nAID, TD_INSERT_CHARACTER_INFO* pInsertCharInfo)
{
	...
	if (!gsys.pDBManager->CharInsert(uidPlayer, nAID, pInsertCharInfo->szName
		...
		, int(pInsertCharInfo->nHair), int(pInsertCharInfo->nFace), pInsertCharInfo->nHairColor, pInsertCharInfo->nSkinColor, int(pInsertCharInfo->nVoice)
...
bool GFakeLoginLogic::MoveToLoginServer(const MUID& uidPlayer)
{
	...
	router.RouteStartMoveLoginServerRes(uidPlayer, CR_SUCCESS, uidConnectionKey);


	//////////////////////////////////////////////////////////////////////////
	pPlayer->OnMoveLoginServer();
