...
void GEntityNPC::_HandleQuestTrigger(const SENSOR_INFO& infoSensor)
{
	...
	class DoHandleQuestTrigger : public GFieldGrid::Sector::IVisitor
	{
	public:
		DoHandleQuestTrigger(GEntityNPC& npc,const SENSOR_INFO&	infoSensor)
			: m_npc(npc)
			, m_infoSensor(infoSensor) {}
	private:
		void OnVisit(const MUID& uid)
		{
			GEntityPlayer* const pPlayer = m_npc.FindPlayer(uid);
			if (NULL == pPlayer) return;

			if (GConst::QUEST_SHARE_DISTANCE < m_npc.DistanceTo(pPlayer)) return;

			gsys.pQuestSystem->GetQObjUpdater().UpdEscortQObj(pPlayer, m_npc.GetID(), m_infoSensor.nID);
		}
	private:
		GEntityNPC&			m_npc;
		const SENSOR_INFO&	m_infoSensor;
	};
	grid_selector->VisitByPos(DoHandleQuestTrigger(*this, infoSensor), ETID_PLAYER, GetPos(), 1);
}
...
void GEntityNPC::SendNPCIcon()
{
	...
	class SendICON : public GFieldGrid::Sector::IVisitor
	{
	public:
		SendICON(GEntityNPC& npc) : m_npc(npc) {}
	private:
		void OnVisit(const MUID& uid)
		{
			GEntityPlayer* const pPlayer = gmgr.pPlayerObjectManager->GetEntity(uid);
			if (NULL == pPlayer) return;

			pPlayer->GetNPCIconSender().SendByNPC(&m_npc);
		}
	private:
		GEntityNPC& m_npc;
	};

	VisitNeighborSectorEntity(SendICON(*this), ETID_PLAYER);
}
