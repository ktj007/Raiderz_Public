...
void GAIEventNotifier::NotifyNeighborSector(const GAIEvent& aiEvent)
{
	GField* const pField = gmgr.pFieldMgr->GetField(aiEvent.uidField);
	if (NULL == pField)
	{
		return;
	}

	GFieldGrid::EntitySelector* const grid_selector = 
		pField->GetGrid().GetEntitySelector();

	class AIEventNotify : public GFieldGrid::Sector::IVisitor
	{
	public:
		AIEventNotify(GAIEventNotifier& notifier,const GAIEvent& aiEvent)
			: m_notifier(notifier)
			, m_aiEvent(aiEvent) {}
	private:
		void OnVisit(const MUID& uid)
		{
			m_notifier.NotifyToUID(uid, m_aiEvent);
		}
	private:
		GAIEventNotifier&	m_notifier;
		const GAIEvent&		m_aiEvent;
	};
	grid_selector->Visit(AIEventNotify(*this, aiEvent), ETID_NPC, MPoint(aiEvent.nSectorX, aiEvent.nSectorY), 1);
}
