...
SUITE(EffectTargetSelector)
{
	struct FEffectTargetSelector : public FBasePlayer
	{
		...
		bool IsEffectTarget(const set<GEntityActor*>& vecEffectTarget, GEntityActor* pTarget)
		{
	...
	namespace EffectPointAndRadius
	{
		...
		namespace Self
		{
			TEST_FIXTURE(FEffectPointAndRadius, RadiusIsZero)
			{
				...
				set<GEntityActor*> vecEffectTarget;
				m_EffectTargetSelector.Select(desc, vecEffectTarget);
