...
SUITE(DBTestNpcKilledLog)
{
	LOGDB_TEST(USP_RZ_KILL_NPC_LOG)
	{
		...
		{
			TDBRecordSet rs;
			UTestDB.Execute(rs, L"SELECT CHAR_PTM, DELTA_CHAR_PTM, LEV FROM dbo.RZ_CHAR_LOG WHERE ACCN_ID = %I64d AND WORLD_ID = %d AND CHAR_ID = %I64d AND CODE = 1801 AND REG_DATE = %s"
				, PLAYER[i].m_nAID, WORLD, PLAYER[i].m_nCID, REG_DATE.c_str());
