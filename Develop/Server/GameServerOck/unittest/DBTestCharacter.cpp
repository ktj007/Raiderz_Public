...
SUITE(RZ_CHARACTER)
{
	GAMEDB_TEST(USP_RZ_CHAR_GET_INFO)
	{
		...
		CHECK(UTestDB.Execute(L"UPDATE dbo.RZ_CHARACTER SET POS_X = %f, POS_Y = %f, POS_Z = %f WHERE CHAR_ID = %I64d;", fPosX, fPosY, fPosZ, nCID));
	...
	GAMEDB_TEST(USP_RZ_CHAR_GET_SIMPLE_INFO)
	{
		...
		CHECK_EQUAL(0, rs.Field(L"FEAT_HAIR").AsByte());
		CHECK_EQUAL(0, rs.Field(L"FEAT_FACE").AsByte());
		CHECK_EQUAL(1, rs.Field(L"FEAT_HAIR_COLOR").AsShort());
		CHECK_EQUAL(1, rs.Field(L"FEAT_SKIN_COLOR").AsShort());
