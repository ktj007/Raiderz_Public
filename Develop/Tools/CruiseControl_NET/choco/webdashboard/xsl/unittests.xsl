...
<xsl:stylesheet
    ...
    <xsl:variable name="unittest__.result.list" select="/cruisecontrol/build/unittest-results"/>
    <!-- <xsl:variable name="unittest__.suite.list" select="$nunit2.result.list//test-suite"/> -->
	<xsl:variable name="unittest__.case.list" select="$unittest__.result.list/test"/>
    <xsl:variable name="unittest__.case.count" select="count($unittest__.case.list)"/>
    <xsl:variable name="unittest__.time" select="$unittest__.result.list/@time"/>
    <!-- <xsl:variable name="unittest__.failedtest.list" select=""/> -->
	<!-- <xsl:variable name="unittest__.failedtest.count" select="$unittest__.result.list/@failedtests"/> -->
	<xsl:variable name="unittest__.failure.list" select="$unittest__.result.list/test/failure"/>
    <xsl:variable name="unittest__.failure.count" select="$unittest__.result.list/@failures"/>
    ...
    <xsl:variable name="total.time" select="$unittest__.time + $nunit2.time + $junit.time"/>
    <xsl:variable name="total.notrun.count" select="$nunit2.notrun.count"/>
    <xsl:variable name="total.run.count" select="$unittest__.case.count + $nunit2.case.count + $junit.case.count - $total.notrun.count"/>
    <xsl:variable name="total.failure.count" select="$unittest__.failure.count + $nunit2.failure.count + $junit.failure.count + $junit.error.count"/>
    ...
    <xsl:template match="/">
        <table class="section-table" cellpadding="2" cellspacing="0" border="0" width="98%">
            ...
            <xsl:apply-templates select="$unittest__.failure.list | $junit.failure.list | $nunit2.failure.list"/>
            ...
            <xsl:if test="$total.failure.count > 0">
				...
				<xsl:call-template name="unittest__testdetail">
                    <xsl:with-param name="detailnodes" select="//test[.//failure]"/> 					
                </xsl:call-template>
	...
	<!-- Unittest++ Test Failures And Warnings Detail Template -->
    <xsl:template name="unittest__testdetail">
        <xsl:param name="detailnodes"/>

        <xsl:for-each select="$detailnodes">
        
            <xsl:if test="failure">
			<tr><td class="section-data">Suite:</td><td class="section-data"><xsl:value-of select="@suite"/></td></tr>
            <tr><td class="section-data">Test:</td><td class="section-data"><xsl:value-of select="@name"/></td></tr>
			<tr><td class="section-data">Time:</td><td class="section-data"><xsl:value-of select="@time"/></td></tr>
            <tr><td class="section-data">Type:</td><td class="section-data"><span style="color: red;">Failure</span></td></tr>
            <xsl:for-each select="failure">
				<tr><td class="section-data">Message:</td><td class="section-data"><xsl:value-of select="@message"/></td></tr>
			</xsl:for-each>
			</xsl:if>  
