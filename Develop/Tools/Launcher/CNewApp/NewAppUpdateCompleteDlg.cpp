...
*************************************************************************************/
// 클래스 생성자
CNewAppUpdateCompleteDlg::CNewAppUpdateCompleteDlg( CWnd* pParent) : CDDBDialog( IDD_UPDATE_CONFIRM, pParent)
{
	//{{AFX_DATA_INIT( CNewAppUpdateCompleteDlg)
	//}}AFX_DATA_INIT
...
// DoDataExchange
void CNewAppUpdateCompleteDlg::DoDataExchange( CDataExchange* pDX)
{
	CDDBDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CNewAppUpdateCompleteDlg)
...
// Message map
BEGIN_MESSAGE_MAP( CNewAppUpdateCompleteDlg, CDDBDialog)
	//{{AFX_MSG_MAP(CNewAppUpdateCompleteDlg)
	ON_WM_TIMER()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CNewAppUpdateCompleteDlg::OnInitDialog() 
{
...
// SetDialog
void CNewAppUpdateCompleteDlg::SetDialog( CDialog* pDialog, CString strCaption, CString strMessage, UINT nButtonType)
{
...
// Timer
void CNewAppUpdateCompleteDlg::OnTimer(UINT nIDEvent)
{
	// 업데이트 타이머
	if ( nIDEvent == 0)
	{
...
// 메시지 출력
void CNewAppUpdateCompleteDlg::PutMessage( int nTime)
{
	// 메시지 표시
	CString strMsg;
