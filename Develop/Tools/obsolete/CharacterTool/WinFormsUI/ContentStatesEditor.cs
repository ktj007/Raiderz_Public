...
{	
	internal class ContentStatesEditor : UITypeEditor
	{
		/// <summary>
		/// Summary description for ContentStatesEditorControl.
		/// </summary>
		private class ContentStatesEditorControl : System.Windows.Forms.UserControl
		{
			...
			private ContentStates m_oldStates;

			public ContentStates States
			{
				get
				{
					ContentStates states = 0;
					if (checkBoxFloat.Checked)
						states |= ContentStates.Float;
					if (checkBoxDockLeft.Checked)
						states |= ContentStates.DockLeft;
					if (checkBoxDockRight.Checked)
						states |= ContentStates.DockRight;
					if (checkBoxDockTop.Checked)
						states |= ContentStates.DockTop;
					if (checkBoxDockBottom.Checked)
						states |= ContentStates.DockBottom;
					if (checkBoxDockFill.Checked)
						states |= ContentStates.Document;
			...
			public ContentStatesEditorControl()
			{
		...
		private ContentStatesEditor.ContentStatesEditorControl m_ui = null;
		...
		public override object EditValue(ITypeDescriptorContext context, IServiceProvider sp, object value)
		{
			...
			if (m_ui == null)
				m_ui = new ContentStatesEditor.ContentStatesEditorControl();

			m_ui.SetStates(edSvc, (ContentStates)value);
