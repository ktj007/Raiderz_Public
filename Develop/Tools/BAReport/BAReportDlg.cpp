...
void SetCommand(LPTSTR pszCmd)
{
	...
	if (strCmd == "/help") {
		AfxMessageBox(L"USAGE_EX) BAReport app=appname;addr=moon.maiet.net;port=21;id=ftp;passwd=ftp@;gid=10;user=moanus;localfile=rslog.txt;remotefile=remote_rslog.txt;agree=0;srcdelete=0;", MB_OK);
		return;
	}

#define MAX_LOCALFILE	10
	wchar_t szLocalFileName[MAX_LOCALFILE][256];
	wchar_t szRemoteFileName[MAX_LOCALFILE][256];

	for (int i = 0; i < MAX_LOCALFILE; i++)
	{
		szLocalFileName[i][0] = 0;
		szRemoteFileName[i][0] = 0;
	}
		...
		if (nPos2 != -1) {
			...
			else if (strName == "localfile")
			{
				...
				while (true) {
					...
					wcscpy_s(szLocalFileName[nLocalFileCount++], strTmp);
				...
				while (true) {
					...
					wcscpy_s(szRemoteFileName[nRemoteFileCount++], strTmp);
...
//	strRemoteFileName.Format("%s_%2d%2d_%s_%s_%s", strApp, time.GetMonth(), time.GetDate(), strGID, strUser, strLocalFile);
	...
	for (int i = 0; i < nLocalFileCount; i++) {
		...
		g_pFileTransfer->PutFileQueue(strAddr, _wtoi((LPCTSTR)strPort), strID, strPasswd, strLocal, strRemote);
	}
